...
public class ClientHandler implements Runnable {
    ...
    private void handleGetFriends(String[] parts) throws IOException {
        ...
        int userId = Integer.parseInt(parts[1]);
        com.iwish.database.FriendsDAO friendsDAO = new com.iwish.database.FriendsDAO(dbManager);
        List<com.iwish.models.Friend> friends = friendsDAO.getFriends(userId);
        
        StringBuilder response = new StringBuilder("SUCCESS##" + friends.size());
        for (com.iwish.models.Friend friend : friends) {
            response.append("##")
     ...
     * Response: SUCCESS##count##friendshipId:username##...
     */
    private void handleGetFriendRequests(String[] parts) throws IOException {
        ...
        int userId = Integer.parseInt(parts[1]);
        com.iwish.database.FriendsDAO friendsDAO = new com.iwish.database.FriendsDAO(dbManager);
        List<com.iwish.models.FriendRequest> requests = friendsDAO.getFriendRequests(userId);
        
        StringBuilder response = new StringBuilder("SUCCESS##" + requests.size());
        for (com.iwish.models.FriendRequest request : requests) {
            response.append("##")
                    .append(request.getFriendshipId()).append(":")
                    .append(request.getRequesterUsername());
        }
    ...
    private void handleAddFriend(String[] parts) throws IOException {
        ...
        com.iwish.database.FriendsDAO friendsDAO = new com.iwish.database.FriendsDAO(dbManager);
        List<com.iwish.models.User> users = friendsDAO.searchUsers(friendUsername);
        
        if (users.isEmpty()) {
            dos.writeUTF("ERROR##User not found");
            return;
        }
        
        int friendId = users.get(0).getUserId();
        if (userId == friendId) {
            dos.writeUTF("ERROR##Cannot add yourself");
            return;
        }
        
        boolean sent = friendsDAO.sendFriendRequest(userId, friendId);
        if (sent) {
            dos.writeUTF("SUCCESS");
        } else {
            dos.writeUTF("ERROR##Request already exists");
        }
    }
    ...
    private void handleAcceptFriend(String[] parts) throws IOException {
        ...
        if (accepted) {
            ...
            dos.writeUTF("ERROR##Failed to accept");
        }
    }
    ...
    private void handleDeclineFriend(String[] parts) throws IOException {
        ...
        if (declined) {
            ...
            dos.writeUTF("ERROR##Failed to decline");
        }
    }
    ...
    /**
     * SEARCH_USERS##query
     * Response: SUCCESS##count##userId:username##...
     */
    private void handleSearchUsers(String[] parts) throws IOException {
        ...
        com.iwish.database.FriendsDAO friendsDAO = new com.iwish.database.FriendsDAO(dbManager);
        List<com.iwish.models.User> users = friendsDAO.searchUsers(query);
        
        StringBuilder response = new StringBuilder("SUCCESS##" + users.size());
        for (com.iwish.models.User user : users) {
            response.append("##")
