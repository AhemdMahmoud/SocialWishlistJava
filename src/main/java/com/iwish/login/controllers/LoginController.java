...
import com.iwish.login.networking.NetworkHandler;
...
public class LoginController {
    ...
    private void handleSignIn() {
        ...
        try {
            ...
            String response = NetworkHandler.getInstance().sendRequest(request);

            if (response != null && response.startsWith("SUCCESS")) {
                showLoginMessage("Login Successful! Redirecting...", false);
                // TODO: Navigate to Main Dashboard
            } else {
                String error = response != null ? response.replace("ERROR:", "") : "Unknown error";
                showLoginMessage(error, true);
            }
        } catch (IOException e) {
            e.printStackTrace();
            showLoginMessage("Network Error: " + e.getMessage(), true);
        }
    }
    ...
    private void handleRegister() {
        ...
        try {
            ...
            String request = "REGISTER:" + username + ":" + pass + ":" + email;
            String response = NetworkHandler.getInstance().sendRequest(request);

            if (response != null && response.startsWith("SUCCESS")) {
                showRegisterMessage("Registration Successful! Please Sign In.", false);
                // Optional: Delay and switch to login, or just let user click "Back to Login"
            } else {
                String error = response != null ? response.replace("ERROR:", "") : "Unknown error";
                showRegisterMessage(error, true);
            }
        } catch (IOException e) {
            e.printStackTrace();
            showRegisterMessage("Network Error: " + e.getMessage(), true);
        }
    }
}
